function getMinutesSinceMidnight(date) {
  ...
  function getNextThreshold() {
    ...
    for (const [start, end, quiet] of transitions) {
      ...
      const endCalc = (end > 1440) ? end % 1440 : end;
  ...
  function updateTheme() {
    // Check if auto dark mode is enabled in settings
    chrome.storage.sync.get(['darkModeAuto'], function(result) {
      const darkModeAuto = result.darkModeAuto !== false; // Default to true if not set
